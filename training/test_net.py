...
def run_frame(im_on, gt_on):
    ...
    if gt_on == 0 or not (
        gt[gt_on, 4] == gt[gt_on - 1, 4] and gt[gt_on, 5] == gt[gt_on - 1, 5] and gt[gt_on, 6] - 1 == gt[gt_on - 1, 6]
    ):
...
if __name__ == "__main__":
    ...
    if FLAGS.record:
        ...
        writer = imageio.get_writer(
            "./video_%02d_%02d_%02d_%02d_%02d.mp4" % (tt.tm_mon, tt.tm_mday, tt.tm_hour, tt.tm_min, tt.tm_sec), fps=FPS
        )
    ...
    for imOn in range(FLAGS.skipCount, maxIter):
        if DISPLAY:
            plots, titles, results = run_frame(imageNums[int(imOn)], int(imOn))
            im = subplot(
                plots,
                NUM_ROWS,
                NUM_COLS,
            ...
            results = run_frame(imageNums[int(imOn)], int(imOn))
