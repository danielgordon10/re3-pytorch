...
def subplot(plots, rows, cols, outputWidth, outputHeight, border=BORDER, titles=None, fancy_text=False):
    returnedImage = np.full(
        ((outputHeight + 2 * border) * rows, (outputWidth + 2 * border) * cols, 3), 191, dtype=np.uint8
    )
    if fancy_text:
        from PIL import Image, ImageDraw, ImageFont

        FANCY_FONT = ImageFont.truetype("/usr/share/fonts/truetype/roboto/hinted/Roboto-Bold.ttf", 20)
    for row in range(rows):
        for col in range(cols):
            ...
            if im.shape != (outputHeight, outputWidth, 3):
                ...
                im = cv2.resize(im, (imWidth, imHeight), interpolation=cv2.INTER_NEAREST)
                if imWidth != outputWidth:
            ...
            if (
                ...
                if fancy_text:
                    ...
                    draw.text((10, 10), titles[col + cols * row], (255, 255, 255), font=FANCY_FONT)
                    im = np.array(im)
                else:
                    cv2.putText(im, titles[col + cols * row], (10, 30), CV_FONT, 0.5, [0, 0, 0], 4)
                    cv2.putText(im, titles[col + cols * row], (10, 30), CV_FONT, 0.5, [255, 255, 255], 1)
            returnedImage[
                border + (outputHeight + border) * row : (outputHeight + border) * (row + 1),
                border + (outputWidth + border) * col : (outputWidth + border) * (col + 1),
                :,
            ] = im
    im = returnedImage
    # for one long title
    if titles is not None and len(titles) == 1:
        if fancy_text:
            ...
            cv2.putText(im, titles[0], (10, 30), CV_FONT, 0.5, [0, 0, 0], 4)
            cv2.putText(im, titles[0], (10, 30), CV_FONT, 0.5, [255, 255, 255], 1)
